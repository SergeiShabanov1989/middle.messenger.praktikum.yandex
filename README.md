...
src/
  ...
  core/          — каркас
    ...
    HTTPTransport.ts         — fetch-обёртка с ApiError и типизированными ошибками
...
- Выбор чата открывает окно переписки с историей сообщений.
- Отправка текстового сообщения (добавляется локально в стор).
...
`HTTPTransport` (`src/core/HTTPTransport.ts`) — обёртка над `fetch`:
- базовый URL `https://ya-praktikum.tech/api/v2`, `credentials: 'include'`
...
В ветке `main` хранятся только исходники; текущая ветка спринта — `sprint_3`.